                             ...
                             unsigned long _n_info_frames)
{
	...
	// After implementing the list the following lines have been commented out and hence directly the pools can be accessed without the use of a dummy class.
 /*   if(info_frame_no == 0) {          
...
if(!info_frame_no){
	...
	nFreeFrames--; // one frame is being used for info frame
}
	//(*ptrTonFreeFrames)--;
    
	
	if(ContFramePool::head==NULL)  // entering the first pool to first and last static pointers
    {
		ContFramePool::head = this;
		ContFramePool::tail = this;
    }
    else                           // else using the last pointer to go on adding new frames
    {
		ContFramePool::tail->next = this;
        ContFramePool::tail = ContFramePool::tail->next;
    }
...
unsigned long ContFramePool::get_frames(unsigned int _n_frames)
{
    ...
    assert(nFreeFrames > _n_frames);   //Check to see if there are requested number of free frames 
...
// This extra function has been commented as the linked list implementation is successful now as given in the previous comments.
...
void ContFramePool::release_frames(unsigned long _first_frame_no)
{
   ...
   // ContFramePool dummy;   // Define a reference that can be used to access the private members of a given pool
	...
	ContFramePool *current= head; // refer to the head pointer to start looping around the pools to check if the requested pool belongs to any
	while(current!=NULL)
    {
    	if(_first_frame_no >= current->base_frame_no && _first_frame_no < (current->base_frame_no+current->n_frames))  
    	{
			
			i = (_first_frame_no - current->base_frame_no)/4;
			n = (_first_frame_no - current->base_frame_no)%4;
			mask = mask >> (n*2);

			//Assert if the first frame is not the head
    		assert(((current->bitmap[i] & mask) == 0x40) || ((current->bitmap[i] & mask) == 0x10) || ((current->bitmap[i] & mask) == 0x04) || ((current->bitmap[i] & mask) == 0x01));
		
			current->bitmap[i] = current->bitmap[i] | (mask);    //  the head should be freed
    		current->nFreeFrames++;
   	        mask = mask >> 2;
    		if(mask == 0x00)
   	        {
			...
			while((current->bitmap[i]&mask)==0x00)      // if head found free its sequence
    		{
    			current->bitmap[i] = current->bitmap[i] | mask;
        		current->nFreeFrames++;
				mask = mask >> 2;
        	    if(mask == 0x00)
				{
			...
			break;  // break if it is the desired pool
		}
		current=current->next;  // if frame not found traverse the next pool and see
    }  	
...
unsigned long ContFramePool::needed_info_frames(unsigned long _n_frames)
{
    ...
    return(_n_frames / ((16 *(1<<10)) + ((_n_frames % (16 *(1<<10))) > 0 ? 1 : 0)) + 1); 
	//If we use 1 bit we can map 32K frames and 16K if 2 bits per frame are used.
